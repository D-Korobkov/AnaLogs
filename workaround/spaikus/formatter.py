...
class Log:
    def __init__(self, id: str, span_id: str, trace_id: str, timestamp: int, data: dict):
        self.id = id
...
class Span:
    def __init__(self, span_id: int, trace: "Trace", name: str, 
                 start_at: int, end_at: int, parent: "Optional[Span]" = None):
        self.id = span_id
        self.trace = trace
        self.name = name
    ...
    def compute_hash(self, ordered=False) -> None:
        ...
        if ordered:
            self.hash = hash((str([hash(n) for n in self.nodes]),
...
class Trace:
    ...
    def compute_hash(self, ordered=False) -> None:
        hashes = [hash(self.spans[span_id]) for span_id in self.spans_order 
...
class DotFormatter:
    ...
    def add_span(self, span_id, trace_id: str, name: str, start_at: int, end_at: int, 
                 parent_id: "Optional[str]"=None, update_on_conflict=False) -> "Span":
    ...
    def set_span_parent(self, span: "Span", parent_id: str) -> None:
        ...
        parent = self.add_span(parent_id, span.trace.id, f"<span_{span.id} parent>", span.start_at, span.end_at, update_on_conflict=False)
        span.parent = parent
        parent.children += [span]

    def add_log(self, span_id: str, trace_id: str, timestamp: int, kwargs: dict) -> "Optional[Log]":        
        ...
        span = self.add_span(span_id, trace_id, f"<log_{log.id} parent>", timestamp, timestamp, None, update_on_conflict=False)
        span.nodes += [log]
        self.log_id += 1
        return log
    ...
    def get_spans_order(self, trace: "Trace", grouped=False) -> "List[str]":
        if grouped:
    ...
    def vis(self, grouped=False) -> str:
        ...
        for trace_id in self.get_traces_order(grouped):
            ...
            for span_id in self.get_spans_order(trace, grouped):
                span = spans[span_id]
